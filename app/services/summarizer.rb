class Summarizer
  ...
  def initialize(record_ids)
    @developments = Development.where(id: record_ids).find_in_batches
  end

  def markers
    @developments.map(&:location).map do |loc|
      "L.marker(#{loc}, {icon: dotIcon})"
    end.join(', ')
  end
...
#footer
  ...
  %ul
    ...
    %li Limited to the first 100 records
...
:javascript
  ...
  var markers = [#{
    @search.developments.map(&:location).
      map { |loc| "L.marker(#{loc}, {icon: dotIcon})" }.join(', ')
  }]
